"""AOI lifecycle — create, status hint, force-refresh, delete."""
...
@router.post("/{aoi_id}/refresh")
async def refresh_aoi(
    aoi_id: str,
    background_tasks: BackgroundTasks,
) -> dict:
    """Force re-fetch of all categories for an existing AOI."""
    meta = get_aoi_meta(aoi_id)
    if meta is None:
        raise HTTPException(status_code=404, detail="AoI not found")
    ...
    for stage_name in STAGE_NAMES:
        meta_path = category_dir(aoi_id, stage_name) / "meta.json"
        if meta_path.exists():
            meta_path.unlink()
    ...
    return {"job_id": job_id, "aoi_id": aoi_id, "refreshing": "all"}


@router.delete("/{aoi_id}")
async def delete_aoi_endpoint(aoi_id: str) -> dict:
    """Delete an AOI and all of its stored data."""
    meta = get_aoi_meta(aoi_id)
    if meta is None:
        raise HTTPException(status_code=404, detail="AoI not found")
