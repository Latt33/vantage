...
export default function OperationsPage() {
  ...
  async function onApplyMissionWindow(cond: MissionConditionsUi) {
    ...
    // Yield to the browser so the spinner paints before the (synchronous)
    // scoring loop kicks in. The artificial floor also keeps very small
    // AOIs from flashing the loader for a single frame.
    await new Promise((r) => setTimeout(r, 450));
    const windows = computeMissionWindows(cond, sourceData.weather);
    setMissionWindows(windows);
    setAnalyzingWindows(false);
    setMissionWindowOpen(false);
  }
...
function computeMissionWindows(
  ...
  while (i < evals.length) {
    ...
    // Each timestep represents the hour starting at runEnd — extend the
    // band to the next timestep boundary (or +1h fallback) so a lone
    // passing sample still renders as a visible 1-hour bar.
